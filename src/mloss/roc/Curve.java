...
public class Curve {
    ...
    /**
     * Computes fmeasure at a particular classification threshold.
     *
     * @param rankNumber The number of elements in the ranking to treat
     * as positive.
     * @return fmeasure at the given threshold.
     */
    public double fmeasure(int rankNumber){
    	// fmeasure=2*r*p/(r+p)
    	double r=recall(rankNumber);
    	double p=precision(rankNumber);
    	return (2*r*p)/(r+p);	
    }
    /**
     * Computes the best fmeasure 
     *
     * @return best fmeasure and the coresponding r and p.
     */
    public double[] bestfmeasure(){
    	double[] output=new double[3];
    	int bestindex=1;
    	double bestf=0.0;
    	for(int i=1;i<=totalPositives+totalNegatives;i++){
    		if(fmeasure(i)>=bestf){
    			bestf=fmeasure(i);
    			bestindex=i;
    		}
    	...
    	output[0]=bestf;
    	output[1]=recall(bestindex);
    	output[2]=precision(bestindex);
    	return output;
    }
